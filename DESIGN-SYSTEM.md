...
**Entry point currently paused**: the `.lang-switch` pill linking each English page to `fa/` is commented out (not deleted) in `.nav-links` on all four English pages. `fa/index.html` itself is untouched and still fully live at its URL — only the discoverable link from the English nav is off. To re-enable, uncomment the single `<a class="lang-switch">` line in each of `index.html` / `services.html` / `about.html` / `book.html`.
