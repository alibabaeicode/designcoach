...
- **Hero word cycle**: the yellow-highlighted word in the Home hero (`.mark-yellow[data-cycle-words]`) rotates through a JSON array of alternates stored in its own `data-cycle-words` attribute (currently: ship / defend / own / repeat / trust), via `initHeroTextCycle()` in `js/main.js` — a plain `opacity` crossfade (0.35s) every 2.6s, `textContent` swapped while faded out. The two fixed lines above it ("Design decisions" / "your team can") are static markup, untouched by the script. No-ops entirely under `prefers-reduced-motion` (first word just stays put, no interval started). To change the word list, edit the JSON in that one attribute — no JS changes needed.
