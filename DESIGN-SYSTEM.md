...
- **Hero word cycle**: in the Home hero's yellow mark, "actually" is static text; only the word after it (`.cycle-word`, wrapped in an overflow-hidden `.cycle-word-mask`) rotates through a JSON array in its `data-cycle-words` attribute (currently: ship / defend / own / repeat / trust), via `initHeroTextCycle()` in `js/main.js`. Each swap: slide up + fade out (`.cycle-out`, 0.5s `cubic-bezier(0.65,0,0.35,1)` transform + 0.4s opacity), then — with the element's transition briefly forced to `none` so the reposition itself doesn't animate — the text is swapped and the element is snapped below the mask (`.cycle-in`, translateY(100%) opacity 0), reflowed, transition restored, and `.cycle-in` removed on the next frame so it slides up into place. This is the general pattern for any future "rotating word" effect: a fixed overflow-hidden mask + one element that slides out one side and back in from the other, never a plain opacity swap in place (tried first, looked flat — this reads much smoother). Two fixed lines above it ("Design decisions" / "your team can") and the word "actually" itself are static markup/text, never touched by the script. No-ops entirely under `prefers-reduced-motion` (first word just stays put, no interval started). To change the word list, edit the JSON in the `data-cycle-words` attribute — no JS changes needed.
