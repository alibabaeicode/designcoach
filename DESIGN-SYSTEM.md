...
Every English page has a Persian counterpart at `fa/<same-filename>.html`, reached via a `.lang-switch` pill in `.nav-links` on both sides (English pages link to `fa/<page>.html`; Persian pages link to `../<page>.html`). This is a second full localization, not a translated fork of the design — same classes, same components, same `css/style.css`/`js/main.js`, same content structure page-for-page. When adding or changing a section on one language's page, mirror the change on its counterpart in the same commit, the same way the two English/Persian portraits or the two booking forms are already expected to stay in sync.
