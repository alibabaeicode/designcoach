...
- `fa/index.html`, `fa/services.html`, `fa/about.html`, `fa/book.html` — the Persian counterparts. See "Persian (fa/) localization" below.
...
- `js/main.js` — mobile nav toggle + booking form (topic chips, submit/success/error handling). Shared verbatim by every page in both languages that has a `[data-booking-form]` block; see the localization note in its own header comment.
- `js/system-loop.js` — the Home hero animation (see "Current trial" above). Shared verbatim by `index.html` and `fa/index.html`.
- `assets/` — `portrait-home.jpg` (About portrait — the Home hero currently uses the animation instead, see "Current trial"), `logo-mark.jpg` (nav mark), `favicon-16x16.png` / `favicon-32x32.png` / `apple-touch-icon.png` (generated from `logo-mark.jpg`, see Imagery).
...
Every English page has a Persian counterpart at `fa/<same-filename>.html`. This is a second full localization, not a translated fork of the design — same classes, same components, same `css/style.css`/`js/main.js`, same content structure page-for-page. When adding or changing a section on one language's page, mirror the change on its counterpart in the same commit, the same way the two English/Persian portraits or the two booking forms are already expected to stay in sync.

**Entry point currently paused**: the `.lang-switch` pill linking each English page to its `fa/` counterpart is commented out (not deleted) in `.nav-links` on all four English pages, while the Vazirmatn font is reconsidered. The fa/ pages themselves are untouched and still fully live at their URLs — only the discoverable link from the English nav is off. To re-enable, uncomment the single `<a class="lang-switch">` line in each of `index.html` / `services.html` / `about.html` / `book.html` (the fa/ pages' own `../<page>.html` switch-back links were never touched and don't need any change).

- **`<html lang="fa" dir="rtl">`** on every fa/ page. The `dir="rtl"` *attribute* (not a CSS property) is what makes the browser auto-mirror grid/flex column order and default text alignment for free — this is why `fa/index.html`'s hero markup is DOM-identical to `index.html`'s (copy first, portrait second): the grid puts the first item at the inline-start, which is the *right* edge in RTL, with zero markup changes.
- **Logical CSS properties, not physical**: every divider border in the base stylesheet (`border-right` on grid-cell dividers, `padding-left`/`border-left` on the brand-tag divider, the pillar-list bullet's `left` position, etc.) was converted to `border-inline-end` / `padding-inline-start` / `inset-inline-start` and so on. In `dir="ltr"` (every English page) these resolve to the exact same physical side as before — zero visual change, verified by screenshot diff — but they now also resolve correctly for `dir="rtl"` with no separate override needed. **Any new directional CSS (a new border-right, padding-left, absolutely-positioned left/right, etc.) must use the logical form**, or it will render on the wrong side on every fa/ page. The one exception left as physical `border-left`/`border-right` is `.consulting-grid`'s outer box, which sets both sides to the same value (direction-agnostic).
- **Font**: `html[lang="fa"]` redefines `--font-display`/`--font-body`/`--font-mono` to `"Vazirmatn"` (loaded via Google Fonts, weights 300/400/500/700/800/900, in each fa/ page's own `<head>` — not loaded on English pages). Body copy renders at `font-weight: 300` (the "باریک"/slim weight the fa design explicitly asked for); headings and a short explicit list of UI elements that need to read as emphasis (buttons, stat numerals, index numbers, list-item numerals) are bumped back to `700` — see the selector list in `css/style.css`. Any new heading-weight or button-like element added to a fa/ page must be added to that list, or it will render thin.
- **Letter-spacing / uppercase reset**: Vazirmatn is a joined (cursive) script — `letter-spacing` breaks the letter joining, and `text-transform: uppercase` is a no-op on Persian glyphs but would still incorrectly shout any embedded Latin (client names, tool names). Both are reset with `html[lang="fa"] * { letter-spacing: normal !important; text-transform: none !important; }` rather than chasing the 30+ individual rules that set them for the English version — this is the one deliberate `!important` in the stylesheet, scoped narrowly to the fa language root, and it's there on purpose; don't "clean it up" into per-selector overrides.
- **Arrow glyphs**: fa/ markup writes the CTA arrow as `←` (not `→`) directly in the HTML wherever the English version has `<span>→</span>` or an inline `→`, since it's simpler and more robust than mirroring a single glyph with `transform: scaleX(-1)`. The hover "nudge" animation direction is flipped to match (`translateX(-5px)` under `html[lang="fa"]`, vs. `+5px` on English).
- **Numerals**: fa/ pages use real Persian digits (۰–۹) directly in the markup wherever the English version has Latin digits (stats, dates, index numbers, indices like "01"). Vazirmatn renders them natively — no CSS or JS needed, just type them.
- **Client/company/institution names**: kept in their original Latin spelling throughout fa/ (client companies, academies, colleges — e.g. "Telewebion", "Amanj Academy", "Rahnema College") since inventing a Persian transliteration risks misspelling a real brand's actual identity. Well-known university names were translated to their standard Persian names (e.g. "Shahid Beheshti University" → "دانشگاه شهید بهشتی") since those are unambiguous and universally recognized. The two Medium article titles stay in English (the linked articles are actually in English) with a short Persian teaser/summary and an inline "· به انگلیسی" note, rather than a translated title that would misleadingly imply a Persian article exists.
- **Booking form**: fully functional in Persian, posting to the same Formspree endpoint. All validator/error copy comes from data attributes already read generically by `js/main.js` (`data-required-message`, `data-invalid-message`, plus `data-required-message` on `[data-topics-group]` and `data-network-error-message` on `[data-booking-form]`, both added to `js/main.js` specifically to remove its last two hardcoded English strings) — never fork the JS to hardcode Persian text, add/adjust the data attribute instead, on both languages' forms if the copy is changing generally.
- **`hreflang` + Open Graph**: every page (both languages) carries `<link rel="alternate" hreflang="en|fa">` pointing at its counterpart plus an `hreflang="x-default"` pointing at the English URL, and `og:locale`/`og:locale:alternate` (`en_US`/`fa_IR`, flipped per language). Keep these four links and two meta tags in sync with the filename whenever a page is renamed or a new page/language pair is added.
