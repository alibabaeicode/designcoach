...
- Logo mark (footer): the same `assets/logo-mark.jpg`, sized relative to `.footer-wordmark`'s text rather than a fixed pixel value, since the wordmark itself scales with `clamp(32px, 4.4vw, 60px)`. The `clamp()` moved from `.footer-wordmark` onto its new wrapper, `.footer-name-row` (`display:flex; align-items:center`); the wordmark's `font-size` is just `inherit`, and `.footer-mark` is `width/height: 0.85em` so both scale together as one unit at any viewport. Any future footer lockup sizing should follow this pattern (size the wrapper, inherit into the text) rather than hardcoding the logo to a breakpoint-specific pixel size.
...
- **Language switch** (`.lang-switch`): a compact yellow pill in `.nav-links` next to the "Book a session" pill — `"فا"` on all four English pages (linking to `fa/`), `"EN"` on the fa page (linking back to `../index.html`). Kept to two letters, not the full "فارسی"/"English", so it reads as a nav icon-button rather than competing with the real nav labels. Fixed `height: 36px` with flex centering (not padding-driven) so it lines up exactly with `.btn-book`'s height regardless of the two buttons' different font-sizes — if `.btn-book`'s padding/font ever changes its rendered height, re-measure and update this fixed height to match rather than eyeballing it.
...
**Structure** (four sections, reusing existing components, not new ones invented for Persian):
1. `.landing-hero` — a new centered, single-column hero variant, defined right after `.btn-secondary` in `css/style.css` next to the other hero rules. Deliberately *not* `.hero-grid` — no split copy/portrait layout, no System Loop animation, just centered eyebrow → h1 (reusing `.hero-h1` and the same yellow word-cycle mechanism, unchanged) → intro copy → centered CTA row. This is a generic reusable component, not fa-scoped, in case a future centered-hero page in either language wants it. Padding is `76px 40px 88px` — top matches `.services-hero`'s `76px` (both are hero-like sections that should sit close to the nav, not the plain `.section`'s `88px`), bottom stays at the section-padding ceiling since there's a CTA row to breathe above the border. Keep both values inside the `72–88px` vertical-padding band from Spacing & grid below — don't drift a hero's top padding back up past `88px` even under a "give it more room" request; adjust the gap between elements instead.
2. `#services` — the exact `.section.pad-lg` / `.section-head` / `.section-lede` / `.grid-2` / `.offer-card` components from the English homepage's "Two ways I work with you", reused verbatim. `meta-right` reads `۰۱ / خدمات`.
3. The Home page's process component (`.process-section` / `.process-head` / `.process-list` / `.process-row` / `.process-num` / `.process-copy`), reused verbatim, sitting between services and the booking form. Six steps translated for meaning, not word-for-word, since the fa page only offers the two consulting/coaching paths rather than the English site's four service areas (step 4's copy reflects that: "وضعیت فعلی تیم، محصول و شیوه‌ی کاری‌تان را می‌سنجیم" rather than a literal "all four areas" reference). `meta-right` reads `۰۲ / فرایند`, continuing the index numbering after Services' `۰۱`.
4. `#book` — the exact `.book-grid` / `.book-intro` / `.book-form-panel` booking form component, reused verbatim, fully functional (see Booking form below).
...
- **Voice — written fresh, not translated**: fa copy is composed directly from Ali's brand/personality (pragmatic, hands-on, allergic to design theory that doesn't ship — see About's "The Context Trap" article and its "turning abstract design principles into decisions teams can ship" framing), not a sentence-by-sentence translation of the English copy. English and fa can and do diverge in structure, examples, and emphasis — matching the English wording is never the goal, matching its intent is. Register is warm, semi-colloquial, and personal: second person `شما` (never switched to intimate `تو`, since this is still a B2B/professional context) paired with colloquial contractions throughout — `می‌تونه` not `می‌تواند`, `می‌ذاریم` not `می‌گذاریم`, `را`→`رو`, `-تان`→`-تون`. Loanwords get swapped for plain Persian where one reads more naturally to a general audience (`ادیت` → `بازبینی`). Avoid literal English-calque constructions (`حرکت دادن معیارها`, `ظرفیت تیم را می‌سازد`) even where they're grammatically correct — if a sentence reads like it started as English, rewrite it as something a Persian speaker would actually say. Keep this register consistent across any future fa content; don't let a partial edit revert a section back to stiffer formal Persian while the rest stays colloquial.
...
- **Hero word-cycle, two fa-specific overrides** (see "Hero word cycle" under Motion for the base mechanism): (1) `.cycle-word-mask`'s `vertical-align: bottom` lines up with Archivo's descender space but sits visibly below Kalameh's baseline — the cycling word read a few pixels lower than "واقعاً". `html[lang="fa"] .cycle-word-mask { vertical-align: baseline; transform: translateY(0.04em); }` fixes it; the `0.04em` nudge was tuned by eye against the rendered baseline, not computed — re-check it visually (screenshot, not just a bounding-box measurement, since Persian diacritics like the "اً" tanween skew a plain `getBoundingClientRect()` reading) if the display font or its metrics ever change. (2) English forces `.cycle-break` (the `<br>` after "actually") onto its own line ≤900px so long words like "DEFEND" don't overflow next to "ACTUALLY" — fa's two-word verbs (دفاع کند, مقیاس کند, اعتماد کند...) fit fine wrapping naturally instead, so `html[lang="fa"] .cycle-break { display: none; }` inside the same breakpoint opts fa back out of the forced break.
- **Numerals**: real Persian digits (۰–۹) directly in the markup wherever the English version has Latin digits (stats, dates, index numbers). Kalameh renders them natively — no CSS or JS needed, just type them. **Exception — hollow/outline numerals**: `.process-num`'s English style (invisible `--cream` fill + thin `-webkit-text-stroke` outline) reads fine for blocky Latin "01"/"02" but turns Persian digits' simpler curves (`۰`'s plain circle especially) into an illegible smudge. `html[lang="fa"] .process-num` overrides this to a solid `--index-gold` fill with the stroke removed. Check any *other* large hollow/outlined numeral component the same way before reusing it in Persian — solid fill, not stroke-outline, is the safe default for Persian display digits.
